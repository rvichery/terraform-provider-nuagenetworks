...
func resourceVCenterCreate(d *schema.ResourceData, m interface{}) error {
	...
	if attr, ok := d.GetOk("disable_network_discovery"); ok {
		DisableNetworkDiscovery := attr.(bool)
		o.DisableNetworkDiscovery = &DisableNetworkDiscovery
	}
	...
	if attr, ok := d.GetOk("host_level_management"); ok {
		HostLevelManagement := attr.(bool)
		o.HostLevelManagement = &HostLevelManagement
	}
	...
	if attr, ok := d.GetOk("auto_resolve_frequency"); ok {
		AutoResolveFrequency := attr.(int)
		o.AutoResolveFrequency = &AutoResolveFrequency
	}
